...
*    - dry convective adjustment is removed to a separate subroutine
*    - MINORIG is input parameters instead of hard-coded Switches
*    - physical constants CPD, CPV, CL, RV, RD, LV0, G, ROWL are input parameter
*    - model parameters are also input parameters:
*          - ELCRIT, TLCRIT, ENTP, SIGD, SIGS, OMTRAIN, OMTSNOW, COEFFR, COEFFS,
...
C Brian:  Use REAL*8 instead of REAL for all floats
      ...
      REAL*8 FT(ND),FQ(ND),FU(ND),FV(ND),FTRA(ND,NTRA)
      REAL*8 UENT(NA,NA),VENT(NA,NA),TRAENT(NA,NA,NTRA),TRATM(NA)
      REAL*8 UP(NA),VP(NA),TRAP(NA,NTRA)
      REAL*8 M(NA),MP(NA),MENT(NA,NA),QENT(NA,NA),ELIJ(NA,NA)
      REAL*8 SIJ(NA,NA),TVP(NA),TV(NA),WATER(NA)
      REAL*8 QP(NA),EP(NA),WT(NA),EVAP(NA),CLW(NA)
      REAL*8 SIGP(NA),TP(NA),TOLD(NA),CPN(NA)
      REAL*8 LV(NA),LVCP(NA),H(NA),HP(NA),GZ(NA),HM(NA)
C BRIAN... lots more input parameters to declare
      INTEGER MINORIG
      REAL*8 CPD, CPV, CL, RV, RD, LV0, G, ROWL
      REAL*8 ELCRIT, TLCRIT, ENTP, SIGD, SIGS, OMTRAIN, OMTSNOW
      REAL*8 COEFFR, COEFFS, CU, BETA, DTMAX, ALPHA, DAMP
      REAL*8 CBMF, DELT, PRECIP, WD, TPRIME, QPRIME
C
C -----------------------------------------------------------------------
C
C   ***                     Specify Switches                         ***
...
C   ***                      the surface layer)                      ***
C
C Brian: dry adjustment removed from this subroutine, MINORIG is now input
C        IPBL=0
C        MINORIG=1
C
C------------------------------------------------------------------------------
...
C
C Brian: these are all input parameters now
C        ELCRIT=.0011
C        TLCRIT=-55.0
C        ENTP=1.5
C        SIGD=0.05
...
C
C Brian:  constants are now input parameters
C      CPD=1005.7
C      CPV=1870.0
C      CL=2500.0
C      RV=461.5
...
C        PRECIP=0.0
        ...
        IFLAG=0
C
C  BRIAN: the dry adjustment code block ( IF(IPBL.NE.0)THEN ) has been removed.
C
...
C
C Brian: TLIFT now takes thermodynamic constants as input
C        CALL TLIFT(P,T,Q,QS,GZ,ICB,NK,TVP,TP,CLW,ND,NL,1)
        CALL TLIFT(P,T,Q,QS,GZ,ICB,NK,
     *             CPD,CPV,CL,RV,RD,LV0,
     *             TVP,TP,CLW,ND,NL,1)
        DO 54 I=NK,ICB
...
C
C Brian: TLIFT now takes thermodynamic constants as input
C        CALL TLIFT(P,T,Q,QS,GZ,ICB,NK,TVP,TP,CLW,ND,NL,2)
        CALL TLIFT(P,T,Q,QS,GZ,ICB,NK,
     *             CPD,CPV,CL,RV,RD,LV0,
     *             TVP,TP,CLW,ND,NL,2)
...
C
C Brian: pass thermodynamic constants as input arguments
C        SUBROUTINE TLIFT(P,T,Q,QS,GZ,ICB,NK,TVP,TPK,CLW,ND,NL,KK)
        SUBROUTINE TLIFT
     *    (P,T,Q,QS,GZ,ICB,NK,
     *     CPD,CPV,CL,RV,RD,LV0,
     *     TVP,TPK,CLW,ND,NL,KK)

        REAL*8 GZ(ND),TPK(ND),CLW(ND),P(ND)
        REAL*8 T(ND),Q(ND),QS(ND),TVP(ND)
        REAL*8 CPD, CPV, CL, RV, RD, LV0
C
C   ***   ASSIGN VALUES OF THERMODYNAMIC CONSTANTS     ***
C
C        CPD=1005.7
